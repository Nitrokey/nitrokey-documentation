...
If you are in doubt which one to choose, you should use OpenPGP. While this page describes the usage of OpenPGP, S/MIME is described `here <smime.html>`__.
...
-  `Generate keys on
   Nitrokey <openpgp-keygen-gpa.html>`__
...
-  `Generate keys on Nitrokey with different algorithm or key
   size <openpgp-keygen-on-device.html>`__
...
-  `Generate keys
   locally <openpgp-keygen-backup.html>`__
...
-  for using `OpenPGP encryption with
   Thunderbird <openpgp-thunderbird.html>`__

-  for using `OpenPGP encryption with
   Outlook <openpgp-outlook.html>`__
