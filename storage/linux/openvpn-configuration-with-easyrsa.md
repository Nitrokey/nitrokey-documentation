...
After creating the `client.crt` file, we plug the Nitrokey Storage 2 device in the CA machine, and import the `.crt` to the Pro 2 device using this command:
...
While we keep the `client.crt` stored on the Nitrokey Storage 2 device, we must retrieve the `chain.crt` file on the client machine, and store it in the adequate directory. We may use `scp` as in the method explained in the server section of this guide. 

#### 7. Configure the client to interact with the Nitrokey

Now back on the client machine, we will plug the Nitrokey Storage and use it to establish the VPN connection with the server.  In general terms, a connection that uses TLS requires multiple certificates and keys for authentication:
