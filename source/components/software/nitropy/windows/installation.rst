...
3. Change to the directory of the ``.exe`` (something like ``cd C:\Users\YOURUSER\Downloads``)
4. Execute ``nitropy-VERSION.exe``

.. note::
   If you're using Microsoft Edge, the downloaded ``.exe`` might be marked as "untrusted" by Windows. To mark it as trusted anyway, click the three dot on the right of the warning, then "keep", then "show more" and finally "keep anyway".
...
You can find information about our package `here <https://winstall.app/apps/NitrokeyGmbH.Nitropy>`__.
