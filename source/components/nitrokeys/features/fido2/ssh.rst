...
Non-Discoverable Credentials (Non-Resident Keys)
~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~

A non-discoverable credential (non-resident key) is the default credential type created when the user registers their Nitrokey with an authentication system that supports FIDO2/WebAuthn.
...
A discoverable credential (resident key) is stored directly on the Nitrokey, including all credential information and metadata required for authentication.
...
2. Open a terminal and create your SSH key. Replace ``"your_comment"`` with a label to identify it (e.g., "Nitrokey GitLab").  
   ...
   or to create a discoverable credential (resident key)
   ...
   .. note::
      ...
      Discoverable credentials (resident keys) can later be listed and imported on another system with:
